...
async function main() {
  ...
  const admin = await prisma.user.create({
    data: {
      ...
      businessType: 'B2B',
      services: ['KUBE_FARM', 'KUBE_PARK', 'KUBE_LAND'],
  ...
  const animalNames = ['Bella', 'Max', 'Luna', 'Charlie', 'Daisy', 'Rocky', 'Molly', 'Duke', 'Coco', 'Bailey'];
  const breeds = ['Friesian', 'Ankole', 'Sahiwal', 'Boran'];
  ...
  for (let i = 0; i < 50; i++) {
    await prisma.animal.create({
      data: {
        ...
        status: i < 3 ? 'SICK' : i === 48 ? 'MISSING' : 'HEALTHY',
        herdId: i < 30 ? herd1.id : herd2.id,
  ...
  const patrol1 = await prisma.patrol.create({
    data: {
      ...
      rangers: ['Patrick Habimana', 'Eric Nkusi', 'Jean Pierre Byiringiro']
    }
  });
  ...
  const patrol2 = await prisma.patrol.create({
    data: {
      ...
      rangers: ['Patrick Habimana', 'Marie Uwera']
    }
  });
